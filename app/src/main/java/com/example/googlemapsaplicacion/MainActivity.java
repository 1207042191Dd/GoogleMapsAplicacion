package com.example.googlemapsaplicacion;
...
public class MainActivity extends AppCompatActivity implements OnMapReadyCallback {
    ...
    public void onMapReady(@NonNull GoogleMap googleMap) {
        ...
        // Modificamos el listener del marcador para mostrar más información
        mMap.setOnMarkerClickListener(marker -> {
            LatLng position = marker.getPosition();
            // Verificamos si es el marcador de ubicación actual
            if (marker.equals(locationMarker)) {
                txtLatLong.setText("Mi Ubicación - Latitud: " + position.latitude +
                        ", Longitud: " + position.longitude);
            } else {
                // Para otros marcadores, mostramos el nombre del lugar
                txtLatLong.setText(marker.getTitle() + " - Latitud: " + position.latitude +
                        ", Longitud: " + position.longitude);
            }
            return false;
        });

        // Habilitamos la ventana de información del marcador con clic largo
        mMap.setOnInfoWindowClickListener(marker -> {
            // Puedes agregar acciones adicionales al hacer clic en la ventana de información
        });
    }
                ...
                if (response.isSuccessful()) {
                    try {
                        ...
                        for (int i = 0; i < lugares.length(); i++) {
                            ...
                            String direccion = lugar.getString("direccion"); // Agregamos la dirección

                            if (lugarCategoria.equals(categoria) &&
                                    (subcategoria == null || lugarSubcategoria.equals(subcategoria))) {
                                LatLng position = new LatLng(lat, lng);

                                // Creamos el marcador con título y snippet
                                MarkerOptions markerOptions = new MarkerOptions()
                                        .position(position)
                                        .title(nombre)
                                        .snippet("Dirección: " + direccion) // Agregamos la dirección como snippet
                        ...
                        Log.e("Error", "Error en la solicitud: ", e);
                    }
                }
            }
